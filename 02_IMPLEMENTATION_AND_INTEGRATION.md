...
테스트를 남기기로 판정된 로직은 실패하는 테스트를 먼저 작성한 뒤 최소 구현으로 통과시키고 정리한다(Red-Green-Refactor). 버그 수정은 재현 테스트를, 테스트가 없는 기존 코드의 동작 변경은 특성화 테스트를 먼저 작성한다. 무엇을 테스트할지는 최소 구현 원칙의 테스트 기준을 그대로 따르며, 정본은 [wf-implement §3.3](./skills/wf-implement/SKILL.md#33-구현)이다.
