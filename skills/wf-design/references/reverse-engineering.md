# 코드 기반 역공학 절차 (Reverse Engineering)

이 절차는 기존 코드는 있으나 승인된 요구사항·설계 기준선이 없거나 문서가 저장소의 실제 상태와 어긋날 때, 코드에서 증거를 수집해 현행 요구사항·설계 초안을 재구성하기 위해 사용한다. [wf-design 현재 상태 조사](../SKILL.md#41-현재-상태-조사)를 체계화한 확장 절차이며 wf-design이 소유한다.

코드는 현재 동작의 근거일 수 있지만 제품 의도나 정식 요구사항 그 자체는 아니다. 역공학 결과는 증거 기반의 **현행 상태 초안**이며, [wf-design 사용자 승인 관문](../SKILL.md#8-사용자-승인-관문)을 거쳐야 정식 요구사항·설계 기준선이 된다. 이 절차는 요구사항이나 설계를 승인하지 않는다.

이 절차에서는 제품 코드를 변경하지 않는다. 분석 중 필요한 코드 변경이 발견되면 별도 항목으로 기록하고, 승인된 기준선을 확보한 뒤 [wf-implement](../../wf-implement/SKILL.md)에서 수행한다. 산출물의 Markdown 구조, 식별자 표기와 하이퍼링크는 [wf-doc 공통 포맷](../../wf-doc/SKILL.md)을 따른다.

## 1. 적용 시점

다음 상황에서 이 절차를 사용한다.

- 기존 코드에 승인된 요구사항·설계 문서가 없을 때 (인수한 시스템, 레거시 저장소)
- 문서가 있으나 코드와 명백히 어긋나 현행 상태를 다시 확정해야 할 때
- 변경 작업을 시작하기 전에 현행 동작·계약·구조를 기준으로 고정해야 할 때
...
- 분석용 계측이나 실험이 필요하면 [wf-design 프로토타입 규칙](../SKILL.md#1-적용-시점)과 동일하게 검증하려는 가설, 범위, 폐기 여부, 성공·실패 판단 기준을 먼저 명시하고, 제품 변경과 분리된 임시·가역 환경에서 수행한다.
...
| 16. 인계 | 추론과 미확정 내용을 wf-design 검토로 전달한다. | 검토 가능한 현행 상태 초안 |

요구사항 도출(11)과 설계 재구성(12)에서 각 항목에 담을 의미는 [wf-design 요구사항 정의](../SKILL.md#42-요구사항-정의)와 [SW 설계](../SKILL.md#43-sw-설계)를 따르되, 항목의 출처는 제품 의도가 아니라 코드 증거임을 명시한다.
...
권장 식별자는 다음과 같다. 문자열 문법과 번호 표기는 [wf-doc 추적 식별자](../../wf-doc/SKILL.md#추적-식별자)의 항목 ID 규칙을 따른다.
...
신뢰도는 [wf-doc 정보 성격 표지](../../wf-doc/SKILL.md#25-본문-작성)(사실·해석·가정·결정·질문·위험)를 대체하지 않는 보조 속성이며, 두 분류는 다음과 같이 대응시킨다.
...
- 현행 요구사항 초안과 현행 SW 설계 초안은 wf-doc의 [`requirements`](../../wf-doc/references/templates.md#요구사항-requirements), [`design`](../../wf-doc/references/templates.md#설계-design) 유형에 `draft` 상태로 매핑한다.
- 증거·관찰 등록부, 모순·공백·질문 목록, 동적 검증 결과처럼 대응하는 wf-doc 문서 유형이 없는 산출물에는 [wf-doc 공통 머리말](../../wf-doc/SKILL.md#23-공통-머리말-작성)과 하이퍼링크·식별자·상태 규칙만 적용한다. 역공학의 동적 검증은 인수 조건 검증이 아니므로 `verification` 유형을 사용하지 않는다.
- 역공학 전용 문서 유형과 `EV`, `OBS`, `RR`, `RD`, `CON`, `GAP` 식별자를 wf-doc에 정식으로 추가하는 것은 필요가 확인될 때 별도로 검토한다.

## 8. 완료 조건과 인계

다음 조건을 모두 만족하면 역공학 결과를 wf-design 검토로 인계할 수 있다. 인계 문서에는 [wf-doc 인계 블록](../../wf-doc/SKILL.md#27-인계와-재개-지점-작성)을 적용한다.
...
인계 후에는 wf-design의 정식 절차가 이어진다. 추론된 현행 요구사항·설계를 제품 의도와 대조해 보완하고, [완료 조건](../SKILL.md#7-완료-조건)을 충족한 뒤 [사용자 승인 관문](../SKILL.md#8-사용자-승인-관문)에서 기준선 승인을 받는다. 승인 시 `RR`, `RD` 항목 중 채택된 것을 정식 `FR`, `NFR`, `AC`, `DES` 식별자로 전환하고, 전환 관계를 추적표에 남긴다. 채택되지 않은 추론은 삭제하지 않고 거부·보류 이유와 함께 보존한다.

복원한 현행 동작을 변경 전에 특성화 테스트로 고정하는 활동은 이 절차가 아니라 [wf-implement 구현 절차](../../wf-implement/SKILL.md#33-구현)가 소유한다. 이 절차는 테스트 분석(10)과 요구사항 도출(11)에서 확인한 검증 가능한 기대 동작을 특성화 대상 선정의 입력으로 인계한다.
