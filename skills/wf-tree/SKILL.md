---
name: wf-tree
description: 작업 계획을 분기마다 가지가 생기는 트리로 구조화하고 전체 계획과 진행 상태를 화면에 시각화하는 스킬. 여러 작업을 묶는 포트폴리오 계층(노드=작업 1개)과 작업 내부의 action item 계층(설계·구현·리뷰·테스트 등)을 함께 다룬다. 계획을 트리로 수립·시각화·갱신하라는 요청, 작업 포트폴리오 관리, 진행 현황을 한눈에 보려는 요청에 사용한다. wf-implement가 계획 수립에서 트리 사용을 결정한 작업의 계획 수립·갱신 시에도 이 스킬의 규칙으로 계획 트리를 생성·재생성한다. 노드 유형별 분기 템플릿으로 누락되기 쉬운 항목(승인 관문·리뷰·롤백 준비 등)을 제안하되, 항목의 의미·채택·완료 판정·상태 전이는 wf-design과 wf-implement가 소유한다. 계획 문서의 형식은 wf-doc을 따른다.
---
...
- [wf-implement 계획 수립](../wf-implement/SKILL.md#32-계획-수립)이 트리 사용을 결정한 계획을 수립·갱신할 때 — TASK 작성·상태 갱신과 같은 변경에서 트리를 생성·재생성한다
...
**완료 스냅숏 예외:** 완료된 사이클의 트리 표현을 작업 기록에 백업한 스냅숏은 살아있는 표현이 아니라 동결된 역사 기록이다. `<!-- generated -->` 대신 `<!-- snapshot: YYYY-MM-DD 완료 시점 -->` 표시를 두고, 이후 재생성·수동 수정하지 않는다. 재생성되는 트리는 계획 문서의 트리 하나뿐이며, 완료부터 축약까지 두 표현이 일시 공존해도 정본 충돌이 아니다. 스냅숏의 기록 시점과 의무는 [wf-implement 작업 기록과 저장 위치](../wf-implement/SKILL.md#7-작업-기록과-저장-위치)가 소유한다.
...
- **완료 상태 보존:** `completed`로 판정된 노드는 이후 요구사항·설계 기준선이 바뀌어도 미완료로 되돌리지 않는다([DCR 변경 원칙](../wf-design/references/design-change.md#1-변경-원칙)). 재작업은 DCR에서 파생된 새 노드(`change` 분기와 후속 작업)로 표현한다.
...
Mermaid 표기 규칙: 상태를 `classDef`(completed·in-progress·pending·gate)로 구분하고, 승인·릴리스 게이트는 이중 테두리(`[[...]]`)와 ★로 표시하며, `depends:` 관계는 점선 간선으로 그린다. 점선은 선행 노드에서 의존하는 노드로 향한다 — 화살표 머리 쪽이 대기자다(의존하는 쪽이 대상을 가리키는 UML 의존성 화살표와 방향이 반대다).

색상별 의미:

| classDef | 색 | 의미 |
|---|---|---|
| `done` | 초록 (`#c8e6c9`/`#2e7d32`) | `completed` — 판정 당시 기준선에 대한 완료 기록. [완료 상태 보존](#6-상태와-롤업)에 따라 기준선이 바뀌어도 되돌리지 않는다 |
| `active` | 노랑 (`#fff9c4`/`#f9a825`) | `in-progress` — 진행 중. 활성 경로 중심 뷰의 펼침 기준 |
| `todo` | 회색 (`#eceff1`/`#90a4ae`) | `pending` — 아직 시작하지 않음 |
| `gate` | 빨강 (`#ffcdd2`/`#c62828`) | 아직 통과하지 않은 필수 관문(승인·릴리스). 상태가 아니라 관문 강조이며, 통과한 관문은 이중 테두리·★를 유지한 채 `done` 색으로 그린다 |

ASCII 트리의 `[✓]`·`[▶]`·`[ ]`가 각각 `done`·`active`·`todo`에 대응한다. `blocked`는 전용 색을 두지 않고 롤업 배지 `⚠ blocked N`으로 표시하며, OR-분기의 기각 대안은 흐리게 그린다.

**완료 시점 표기(ASCII):** 완료로 렌더되는 라인 — `[✓]` action item, 완료된 작업 노드, 접힌 완료 서브트리 롤업 — 은 우측 끝에 완료 시점(`YYYY-MM-DD HH:MM`)을 표기한다.

- 값은 항목 목록의 완료 필드([wf-doc plan 템플릿](../wf-doc/references/templates.md#구현-계획-plan)의 `완료:` 필드, [status 템플릿](../wf-doc/references/templates.md#포트폴리오-status)의 `완료` 열)에서 파생한다. 트리에만 존재하는 시점을 만들지 않는다(단일 소스 원칙). 완료 판정과 필드 기입은 내용 소유자가 한다.
- 기존 우측 주석(작업 노드의 `completed (n/m)` 롤업, `depends:`)이 있으면 완료 시점을 그 뒤 라인의 마지막 요소로 둔다. 좌측 내용과는 점선 리더 또는 공백으로 구분하고 같은 형제 블록에서 열을 맞춘다.
- 접힌 완료 서브트리와 완료 작업 노드의 롤업 시점은 자식 완료 시점의 최댓값을 쓰는 표시 전용 파생값이다. 항목 자체에 완료 필드가 있으면(축약된 작업 등) 그 값을 우선한다.
- 완료 필드가 없는 완료 항목(규칙 신설 전 항목 등)은 시점 표기만 생략하고 렌더링을 계속한다. 자식 중 완료 필드가 없는 항목이 있으면 롤업 시점도 생략한다.
...
- **완료 후 재개와 신설:** `completed` 상태의 포트폴리오에 같은 목표의 연속인 작업(요구사항·설계 변경, 핫픽스 등)이 추가되면 새 문서를 만들지 않고 기존 문서를 `in-progress`로 재개한다. 재개 사유와 추가된 작업은 [wf-doc 변경 이력 규칙](../wf-doc/references/templates.md#공통-변경-이력-절)에 따라 기록한다(절이 없으면 추가한다). 목표 자체가 새로우면 별도 포트폴리오(`docs/status-<슬러그>.md`)를 만든다.
